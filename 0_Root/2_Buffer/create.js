...
/*  
    ...
    Buffer - về mặc định nó là lớp con của Unit8Array , có thể sử dụng các method mở rộng từ Unit8Array 

*/
// -----------------------------------------------------------------------------

// Creates a zero-filled Buffer of length 10 - tạo ra 
const buf1 = Buffer.alloc(10); // <Buffer 00 00 00 00 00 00 00 00 00 00>

// Creates a Buffer of length 10, filled with bytes which all have the value `1`=> Tạo 1 Buffer với chiều dài 10, điền đầy đủ mỗi bytes với giá trị là `1` 
...
/**
    ...
    * Buffer instance might contain old data that needs to be overwritten using fill(), write(), or other functions that fill the Buffer's contents.
    ...
    Tạo bộ đệm chưa khởi tạo có độ dài 10.
    Điều này nhanh hơn gọi Buffer.alloc () nhưng trả về
    Phiên bản đệm có thể chứa dữ liệu cũ cần
    bị ghi đè bằng cách sử dụng fill (), write () hoặc các hàm khác lấp đầy bộ đệm
    các nội dung.
 */
